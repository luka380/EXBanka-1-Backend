...
// errReplicaMalformed is returned by handleReplicaEvent when the event payload
// cannot be parsed. The Start loop uses errors.Is to skip retries for
// unparseable events.
var errReplicaMalformed = errors.New("malformed event payload")

// defaultReplicaBackoff is the sleep schedule between retry attempts (2 sleeps → 3 total attempts).
var defaultReplicaBackoff = []time.Duration{200 * time.Millisecond, 400 * time.Millisecond}
...
// ClientReplicaConsumer maintains stock-service's local client_replica from
...
func NewClientReplicaConsumer(brokers string, repo replicaUpserter) *ClientReplicaConsumer {
	r := kafka.NewReader(kafka.ReaderConfig{
		...
		GroupID:     "stock-service-client-replica",
	})
	return &ClientReplicaConsumer{reader: r, repo: repo, backoff: defaultReplicaBackoff}
}

// handleReplicaEvent parses one event payload and upserts the replica. Separated
// from the read loop so it can be unit-tested without Kafka.
// Returns errReplicaMalformed (wrapped) when the payload cannot be parsed, so
// callers can distinguish parse failures from transient upsert errors.
func (c *ClientReplicaConsumer) handleReplicaEvent(ctx context.Context, value []byte) error {
	var evt kafkamsg.ClientCreatedMessage
	if err := json.Unmarshal(value, &evt); err != nil {
		return fmt.Errorf("%w: %v", errReplicaMalformed, err)
	}
...
// handleReplicaWithRetry calls handleReplicaEvent and retries on transient
// (non-malformed) errors. Malformed payloads are logged and returned immediately
// without retrying. Retries up to len(c.backoff)+1 total attempts, sleeping
// c.backoff[i] between attempts. Each sleep honours ctx cancellation.
func (c *ClientReplicaConsumer) handleReplicaWithRetry(ctx context.Context, value []byte) error {
	maxAttempts := len(c.backoff) + 1
	var lastErr error
	for attempt := 0; attempt < maxAttempts; attempt++ {
		err := c.handleReplicaEvent(ctx, value)
		if err == nil {
			return nil
		}
		if errors.Is(err, errReplicaMalformed) {
...
func (c *ClientReplicaConsumer) Start(ctx context.Context) {
	go func() {
		for {
			...
			if err := c.handleReplicaWithRetry(ctx, msg.Value); err != nil {
