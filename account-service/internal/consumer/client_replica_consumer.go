...
// ClientReplicaConsumer maintains account-service's local client_replica from
...
func NewClientReplicaConsumer(brokers string, repo replicaUpserter) *ClientReplicaConsumer {
	r := kafka.NewReader(kafka.ReaderConfig{
		...
		GroupID:     "account-service-client-replica",
	})
	return &ClientReplicaConsumer{reader: r, repo: repo, backoff: defaultBackoff}
}

// handleReplica parses one event payload and upserts the replica. Separated from the
...
func (c *ClientReplicaConsumer) handleReplica(ctx context.Context, value []byte) error {
...
// handleReplicaWithRetry calls handleReplica and retries on transient (non-malformed) errors.
