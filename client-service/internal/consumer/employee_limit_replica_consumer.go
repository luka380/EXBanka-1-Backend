...
// EmployeeLimitReplicaConsumer maintains client-service's local employee_limit_replica
// from user.employee-limits-updated events (SP-2b).
...
func NewEmployeeLimitReplicaConsumer(brokers string, repo replicaUpserter) *EmployeeLimitReplicaConsumer {
	r := kafka.NewReader(kafka.ReaderConfig{
		...
		GroupID: "client-service-employee-limit-replica",
	})
