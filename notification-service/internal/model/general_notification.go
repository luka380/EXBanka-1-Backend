...
type GeneralNotification struct {
	...
	// SystemType is the recipient namespace: "client" (or empty for legacy rows,
	// treated as client) or "employee" (the shared bank inbox). Reads are scoped
	// by it so a client's notifications never reach an employee/admin with a
	// colliding numeric id, and vice versa.
	SystemType string `gorm:"size:10;not null;default:''" json:"system_type"`
